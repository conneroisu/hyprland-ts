...
function createValidationResult<T>(
  ...
  errors: readonly string[] | undefined = undefined,
): ValidationResult<T> {
  return {
    success,
    data,
...
export const isHyprlandWindow: TypePredicate<HyprlandWindow> = (
  value,
): value is HyprlandWindow => {
  if (!isObject(value)) return false;
...
export const isHyprlandEvent: TypePredicate<HyprlandEvent> = (
  value,
): value is HyprlandEvent => {
  if (!isObject(value)) return false;
...
export const validateHyprlandWindow = (value: unknown): ValidationResult<HyprlandWindow> => {
  ...
  if (!isObject(value)) {
    return createValidationResult(false, undefined, ['Value must be an object']);
  }
  ...
  return createValidationResult(true, value as unknown as HyprlandWindow);
};
...
export const validateHyprlandWorkspace = (value: unknown): ValidationResult<HyprlandWorkspace> => {
  ...
  return createValidationResult(true, value as unknown as HyprlandWorkspace);
};
...
export const validateHyprlandMonitor = (value: unknown): ValidationResult<HyprlandMonitor> => {
  ...
  return createValidationResult(true, value as unknown as HyprlandMonitor);
};

/** Validate array of HyprlandWindow objects */
export const validateHyprlandWindowArray = (value: unknown): ValidationResult<readonly HyprlandWindow[]> => {
  if (!Array.isArray(value)) {
    return createValidationResult(false, undefined, ['Value must be an array']);
  }
  ...
  return createValidationResult(true, validatedWindows as readonly HyprlandWindow[]);
};

/** Validate array of HyprlandWorkspace objects */
export const validateHyprlandWorkspaceArray = (value: unknown): ValidationResult<readonly HyprlandWorkspace[]> => {
  if (!Array.isArray(value)) {
    return createValidationResult(false, undefined, ['Value must be an array']);
  }
  ...
  return createValidationResult(true, validatedWorkspaces as readonly HyprlandWorkspace[]);
};

/** Validate array of HyprlandMonitor objects */
export const validateHyprlandMonitorArray = (value: unknown): ValidationResult<readonly HyprlandMonitor[]> => {
  if (!Array.isArray(value)) {
    return createValidationResult(false, undefined, ['Value must be an array']);
  }
  ...
  return createValidationResult(true, validatedMonitors as readonly HyprlandMonitor[]);
};